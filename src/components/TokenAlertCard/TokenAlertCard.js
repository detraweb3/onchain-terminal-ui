  ...
  return (
    <div style={{ width: "100%", height: "100%", backgroundColor: "#131722", border: "0.25px solid gray" }}>
      ...
      <table style={{width: "100%"}}>
        ...
        {tokens.map((token) => {
          return (
            <tr style={{backgroundColor: "#3e3e3e", borderSpacing: 0, cursor: "pointer"}} onClick={() => {
  ...
  return (
    <div className="modal">
      <div className="modal-content" ref={modalRef}>
        ...
        <p>Buy/Sell Honeypot: {!token.honeypot.buy ? "Yes" : "No"} / {!token.honeypot.sell ? "Yes" : "No"} </p>
        <p>Buy/Sell Tax: {(token.taxes.buyTax * 100).toFixed(2)}% / {(token.taxes.sellTax * 100).toFixed(2)}%</p>
        <p>Number of Buys/Sells: {(token.trading.buys)} / {(token.trading.sells)}</p>
        ...
        <div>
          {Object.keys(token.links).map((i) => {
            return <p><a style={{color: "#a793fa"}} href={token.links[i]}>{i}</a></p>
          })}
          {Object.keys(token.contractLinks).map((i) => {
            return <p><a style={{color: "#a793fa"}} href={token.contractLinks[i]}>{i}</a></p>
          })}
        </div>
